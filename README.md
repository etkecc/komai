...
```bash
sudo pacman -Syu komai
```

The development version is available in the AUR:

```bash
yay -S komai-git # Or your favorite way of installing AUR packages
```

Komai requires a secret server to run, so you'll need to install and configure a service such as [KDE Wallet](https://wiki.archlinux.org/title/KDE_Wallet) or [GNOME Keyring](https://wiki.archlinux.org/title/GNOME/Keyring) if not provided by your desktop environment.

#### Debian (10 and above) / Ubuntu (18.04 and above)

```bash
sudo apt install komai
```

#### Fedora
```bash
sudo dnf install komai
```
...
nix-env -iA nixpkgs.komai

# In an ephemeral shell: (recommended if you just want to try it out without committing to a full installation)
nix-shell -p komai --run komai
# Note: The above command will both install and run Komai.
# To stop it from running immediately, just remove the `--run komai` from the end.
```
...
```
flatpak install flathub im.komai.Komai
```
...
Install komai via the `Discover` app in Desktop Mode (this installs the flatpak). To also make it work in Game Mode you'll have create a wrapper script that starts kwalletd and then komai. You can create `/home/deck/komai.sh` with the following content and then add this script as a "Non-Steam Game" to Steam.
```bash
#!/bin/sh
kwalletd6&
flatpak run --env=XDG_CURRENT_DESKTOP=KDE --env=KDE_SESSION_VERSION=5 --branch=stable --arch=x86_64 --command=im.komai.Komai --file-forwarding im.komai.Komai @@u @@
```
...
komai does not provide binaries for any D-Bus plugins. However, we do provide the following list of known plugins:

- [komai-krunner](https://github.com/LorenDB/komai-krunner)
- [rofi-komai](https://mzte.de/git/LordMZTE/rofi-komai)
...
**Q:** Does komai support end-to-end encryption (EE2E)?
...
**A:** Komai uses Qt's image plugins to render images. You might need to install
...
Komai can use bundled version for most of those libraries automatically, if the versions in your distro are too old.
...
#### Debian 13 [Testing/Sid] (Komai QT6 Version)
...
#### Debian 11-12 / Ubuntu 22.04 (Komai QT5 11.3 Release Version) 
...
```bash
sudo dnf builddep komai # note that some newer packages might not be captured by that
```
...
The `komai` binary will be located in the `build` directory.

#### Windows

After installing all dependencies, you need to edit the `CMakeSettings.json` to
be able to load and compile komai within Visual Studio.
...
Now right click into the root komai source directory and choose `Open in Visual Studio`.
You can choose the build type Release and Debug in the top toolbar.
After a successful CMake generation you can select the `komai.exe` as the run target.
...
```cmd
cd <path-to-komai>\build-vc\Release\Release
windeployqt komai.exe
```
