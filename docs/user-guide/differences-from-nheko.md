...
- **Smarter direct chat detection** -- Komai combines [`m.direct`](https://spec.matrix.org/v1.17/client-server-api/#mdirect) account data, server-declared service members ([MSC4171 `m.member_hints` / `io.element.functional_members`](https://github.com/matrix-org/matrix-spec-proposals/pull/4171), used by mautrix bridges, hookshot, etc.), and member-count heuristics to (hopefully in most cases) correctly identify direct chats, including 3-member rooms where one member is a bridge bot -- giving them the right name, avatar, and room-list placement
...
- **Touch swipe gestures removed** to keep the desktop timeline predictable
...
- **Client-side image metadata stripping** -- by default, EXIF (including GPS), IPTC, and XMP metadata is stripped from JPEG/PNG/WebP attachments before they're uploaded to the homeserver. Toggle in Settings → Composer → Attachments. See [📎 Attachments](features/attachments.md)
...
- **More resilient local data behavior** -- Komai keeps runtime data scoped per profile, cleans room-local media cache data more aggressively when you leave rooms, and treats incompatible local Matrix state as rebuildable local data instead of a reason to wipe the whole profile.
