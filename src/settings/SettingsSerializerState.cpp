...
stageState(const UserSettings &settings, ::komai::rust::SettingsProfileHandle &profileHandle)
{
    ::komai::rust::SettingsStateSnapshot snapshot{
      ...
      .global_excludes                          = {},
      .unread_indicators_hidden_filters         = {},
      .hidden_pins                              = {},
      .hidden_widgets                           = {},
      .collapsed_spaces                         = {},
      .hidden_spaces                            = {},
      ...
      .desktop_system_tray_first_close_prompted = settings.desktopSystemTrayFirstClosePrompted(),
    };
    for (const auto &value : settings.globalExcludes())
        snapshot.global_excludes.push_back(value.toStdString());
    for (const auto &value : settings.unreadIndicatorsHiddenFilters())
        snapshot.unread_indicators_hidden_filters.push_back(value.toStdString());
    ...
    for (const auto &value : settings.pinnedTabs())
        snapshot.pinned_tabs.push_back(value.toStdString());
