...
Each `.yml` file defines 16 Qt palette colors and 5 app-level semantic colors (including the paired `attention` and `attentionText` for unread-mention badge backgrounds and the text shown on them), plus a required `userColors` section for bubble/user colors. Each `userColors` slot is a mapping with required `background` and optional `text`, `secondaryText`, and `link`. `secondaryText` is for in-bubble secondary/inactive content, not outer timeline metadata. At build time, these files are embedded into the Rust binary via `include_str!` and parsed at startup.
...
To hand-craft a theme: create a `.yml` with all 21 palette keys and a `userColors` section (see [Theme YAML format](../../docs/user-guide/features/themes.md#-theme-yaml-format)) and place it here, then `just build`.
