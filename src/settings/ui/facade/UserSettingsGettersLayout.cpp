...
#include "settings/ui/facade/UserSettingsPage.h"
    ...
    int UserSettings::communityListWidth() const { return communityListWidth_; }
    int UserSettings::roomListWidth() const { return roomListWidth_; }
    double UserSettings::scaleFactor() const { return scaleFactor_ > 0.0 ? scaleFactor_ : 1.0; }
    double UserSettings::fontSize() const
    {
    ...
    QString UserSettings::font() const
    {
        if (const auto value =
    ...
    QString UserSettings::emojiFontFamily() const
    {
        if (const auto value =
    ...
    UserSettings::Presence UserSettings::presence() const
    {
        if (const auto value =
              coreStore_.valueAs<int>(settings::core::SettingId::NetworkPresenceStatusPolicy);
            value.has_value() && *value >= static_cast<int>(UserSettings::Presence::AutomaticPresence) &&
            *value <= static_cast<int>(UserSettings::Presence::Offline))
            return static_cast<UserSettings::Presence>(*value);
        return presence_;
    }
    UserSettings::ShowImage UserSettings::showImage() const
    {
        if (const auto value =
