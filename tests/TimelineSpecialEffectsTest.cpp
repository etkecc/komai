...
testContentTriggeredEffects()
{
    ...
    const auto love =
      timeline::effects::detect(mtx::events::collections::TimelineEvents{makeTextEvent("aww ❤️")});
    ok &= expect(containsEffect(love, timeline::effects::SpecialEffect::Love),
                 "heart emoji triggers love effect");

    const auto loveVariant =
      timeline::effects::detect(mtx::events::collections::TimelineEvents{makeUnknownEvent("sweet 😍")});
    ok &= expect(containsEffect(loveVariant, timeline::effects::SpecialEffect::Love),
                 "affection emoji triggers love effect");
    ...
    const auto combined = timeline::effects::detect(
      mtx::events::collections::TimelineEvents{makeTextEvent("🎉☀️❤️⛈🦁")});
    ...
    ok &= expect(containsEffect(combined, timeline::effects::SpecialEffect::Love),
                 "combined body keeps love effect");
...
testEffectNamesFollowInsertionOrder()
{
    ...
    timeline::effects::appendUnique(effects, timeline::effects::SpecialEffect::Love);
    ...
    bool ok = true;
    ok &= expect(names.size() == 5, "five effect names are returned");
    ...
    ok &= expect(names.at(2) == QStringLiteral("love"),
                 "third effect name matches insertion order");
    ok &= expect(names.at(3) == QStringLiteral("lightning"),
                 "fourth effect name matches insertion order");
    ok &= expect(names.at(4) == QStringLiteral("komaiLogo"),
                 "fifth effect name matches insertion order");
    return ok;
}
