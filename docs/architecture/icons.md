...
- `bin/icons/`: icon tooling (`audit.sh`, `fetch.sh`, `sync-fluent.sh`, lock/ref files).
...
- `just icons-fetch <REL_PATH> <ALIAS_SVG_NAME>`: fetch one Fluent icon into `resources/icons/fluent/` and add/update qrc alias (`icons/ui/*.svg` by default).
- `just icons-sync`: sync mirrored Fluent icons from pinned upstream ref.
...
- `bin/icons/fluent-icons.lock`:
  - `FLUENT_REPO` (for example `microsoft/fluentui-system-icons`)
- `bin/icons/FLUENT_VERSION_REF`:
  - pinned upstream version/tag/commit ref
  - value is used as-is (for example `1.1.319`)

Sync scope:

- `just icons-sync` updates every `*.svg` under `resources/icons/fluent/`.
- For each local file, the path relative to `resources/icons/fluent/` is treated as the upstream path.
- `just icons-sync` also regenerates derived local icons (currently `resources/icons/ui/double-checkmark.svg`) from pinned Fluent sources.

## Add/Update Workflow

1. Pin/update ref in `bin/icons/FLUENT_VERSION_REF`.
2. Place mirrored Fluent files under `resources/icons/fluent/` (upstream-relative paths).
3. Run `just icons-sync` (or `just icons-sync --dry-run` first).
