...
komai - Desktop client for Matrix using Qt and C++20
...
+
Log levels can also be set in the KOMAI_LOG_LEVEL environment variable, using
...
+
The log type can also be set in the KOMAI_LOG_TYPE environment variable,
...
same time and start multiple instances of komai. Use _default_ to start with the
default profile.
...
To manage image packs in komai, go to the room settings and enter the Sticker &
...
Hide text and set an optional spoiler warning. Note that Komai does not display
...
Komai by default disables the qml disk cache to prevent crashes. This allows
you to reenable it at your own risk.

== FILES

*Configuration file*::
`${XDG_CONFIG_HOME:-~/.config}/komai/komai.conf`

*Log file*::
`${XDG_CACHE_HOME:-~/.cache}/komai/komai/komai.log`

*Database*::
`${XDG_DATA_HOME:-~/.local/share}/komai/komai/*/data.mdb`

*Media cache*::
`${XDG_CACHE_HOME:-~/.cache}/komai/komai/media_cache`


== REPORTING BUGS

Please report issues on our bug tracker at
<https://github.com/Komai-Reborn/komai/issues>.
