...
- `just settings-generate-3-layer-mapping` - regenerate `docs/architecture/configuration/3-layer-mapping.md` (`SettingId` ↔ runtime getter ↔ persisted key audit)
- `just settings-check-3-layer-mapping` - check drift for that report without rewriting it
